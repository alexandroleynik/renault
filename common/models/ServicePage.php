...
use common\models\query\ServicePageQuery;
 ...
 * @property ServicePageCategory $category
 * @property ServicePageAttachment[] $servicePageAttachments
 */
class ServicePage extends \yii\db\ActiveRecord
{
    ...
    {
        return new ServicePageQuery(get_called_class());
    }
    ...
    public function behaviors()
    {
        return [
            ...
            [
                ...
                'multiple'       => true,
                'uploadRelation' => 'servicePageAttachments'
            ],
            [
                'class'            => UploadBehavior::className(),
                'attribute'        => 'thumbnail',
    ...
    public function rules()
    {
        return [
            ...
            [['category_id'], 'exist', 'targetClass' => ServicePageCategory::className(), 'targetAttribute' => 'id'],
            [['author_id', 'updater_id', 'status', 'weight', 'service_id', 'domain_id'], 'integer'],
    ...
    public function beforeSave($insert)
    {
        if (parent::beforeSave($insert)) {
            ...
            if ($this->slug and $this->service_id and $this->isNewRecord and 'extend'
                != $this->on_scenario) {
    ...
    {
        $model = ServicePage::find()->andWhere([
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if (!empty($this->categoriesList)) {
            ...
            foreach ($this->categoriesList as $categoryId) {
                $model                  = new ServicePageCategories();
                $model->service_page_id = $this->id;
    ...
    {
        return $this->hasMany(ServicePageCategories::className(), ['service_page_id' => 'id']);
    }

    /**
     * @return \yii\db\ActiveQuery
     */
    public function getServicePageAttachments()
    {
        return $this->hasMany(ServicePageAttachment::className(), ['service_page_id' => 'id']);
    }
